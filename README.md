...
1. **ID del Google Sheet** (para el script `Code.gs`). Ya lo dejé configurado con tu ID: `1n13J0V_rqKUWU-6y960H6ODS_leAy9mpImI33n9-gZY`.
...
## ¿Necesito también un `doGet`?

- Para **guardar datos**, solo necesitas `doPost` (eso ya está implementado).
- `doGet` **no es obligatorio** para grabar registros, pero sí útil para validar que el endpoint está vivo desde el navegador.
- En `apps-script/Code.gs` te dejé ambos: `doGet` (health check) y `doPost` (inserción en hoja).
